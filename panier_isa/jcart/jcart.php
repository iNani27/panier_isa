...
class Jcart {
	...
	*/
	private function add_item($id, $name, $price, $qty = 1, $url) {
		...
		if ($validPrice !== false && $validQty !== false) {
			...
			/* Correction Vérifier si $this->qtys[$id] existe avant de vérifier sa valeur  */
	...
	*/
	public function display_cart() {
		...
		// Update a single item ::: Correction isset
		...
		if ($isCheckout !== true) {
			if ($config['button']['checkout']) {
				...
				$src = " src='{$config['button']['checkout']}' alt='{$config['text']['checkout']}' title='' ";
                        ...
                        /* Correction $src pour utilisation dans ligne suivante: echo tab(7) ::: si $src n'existe pas alors on le crée ::: c.a.d. (else) lui donner la valeur "" (rien) */
                        else {$src="";}
...
@session_start(); /* CF2m: probablement util à AJAX ::: Démarrage de session pour l'Ajax, on met l'@ pour éviter l'affichage d'erreur lors d'une précédente ouverture de session */

// Initialize jcart after session start

/* Correction si existe
 * cdt ternaire 
 * (boolean)? si vrai : si faux
 *    */
/*(isset($_SESSION['jcart'])) ? $jcart = $_SESSION['jcart']: $jcart = 'false' ;*/
